...
These are the ten draft boards from the [feasibility study](../bingo-feasibility-2026-09-26/README.md), plus the original preview. Their meanings retain the study's limitations and are labelled as drafts. They have not acquired semantic publication certificates.
...
node --test --test-isolation=none tests/bingo-preview.test.ts tests/bingo-catalog.test.ts
npm run lint
npm run build
```
...
The eight focused tests check URL handling, all ten boards and their stored routes at 3/4/5 lives, corrupted/stale payload rejection, the original preview, and separation from daily publication. Production browser checks use the `/wyrmle/` base path and phone/desktop viewports; see [browser-check.json](browser-check.json) for the exact checked flows. Screenshots in this directory show the picker, public Settings entry, the boards and the five-life animation.
