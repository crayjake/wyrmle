...
The resource row uses a small heart beside the segmented wyrm for Lives and finite reserve counts for REFILLS. Each mini tile has a top-left count and a centred letter; the trailing REFILLS label and all six ANGER groups fit on one line at 320px and 375px. Glyphs are centred using the loaded font's visible ink bounds, including late font loads. The meter and reserve counts retain accessible text descriptions. Earlier designs and the old deployed-site comparison remain under `artifacts/supply-mockups`; those images are historical mockups, not the source of game state. The queue's order remains hidden.
...
`WyrmDecoder.tsx` keeps the `waiting → enemy → tiles → ready` sequence. Enemy cells decode first, then one continuous, distance-sampled route carries the wyrm through every tile with smooth row turns. Layout coordinates are cached until the viewport, element sizes or fonts change, so reveal bounces cannot move the route. In letter combat, the moving wyrm renders the same `WyrmLifeMeter` at the same size as the resting lives, then eases into its exact position before play begins. The static meter reserves its space while decoding; reduced motion reveals both stages quickly without travelling. Resumed games show the meter immediately.
