...
**September 26 and 27 publish the same audited CHAOS v12 challenge** (`letter-strike-7`): five lives, armoured A and O, Life R, Hit A, Revive A and twelve finite refills. Its frozen table contains all **10,351** accepted supply spellings, with every source sense reviewed before publication. The board offers **65 familiar counter spellings across 39 lemmas**, including 49 multi-hit openings. All **3,060 opening spellings / 8,041 physical selections** have replayed familiar winning continuations. See the [assessment review](docs/meaning-assessment-review.md) and [complete publication artifact](artifacts/meaning-v3/selected.json.gz). Played saves retain their original version.

**September 25 retains CHAOS v11** (`letter-strike-7`): five lives, armoured A, Life R, Hit A, Revive A and 20 refill tiles. It has no grammar or length bonuses. Its frozen table stores definitions and classifications for all **29,719** accepted words that fit the total letter supply. The [selected review artifact](artifacts/meaning-v2/selected.json.gz) retains the exact encounter, solver evidence and opening certificate; the [publication review](docs/meaning-coverage-review.md) explains the selection and semantic checks. Every **3,060 opening spelling and all 8,041 physical tile selections** have a replayed familiar winning continuation, with no unsafe or unknown openings. This guarantees recovery after the first word; later choices can still lose.
...
**Settings → Beta puzzles** opens the public [preview picker](https://crayjake.github.io/wyrmle/?preview=bingos) in production as well as development. It offers ten new draft boards with three, four or five lives. Individual links use `?preview=bingo-chaos-1&lives=3` (also `chaos-2`, `anger-1` through `anger-4`, `cruelty-1`/`cruelty-2`, and `fear-1`/`fear-2`, all with the `bingo-` prefix). The original `?preview=bingo` link remains available. See [phone preview instructions](artifacts/bingo-previews-2026-09-26/README.md).
