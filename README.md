...
The resource row puts matching 11px **LIVES** and **REFILLS** labels above the segmented wyrm and reserve tiles. Each mini tile has a top-left count and an optically centred letter; all six ANGER groups fit on one line at 320px and 375px. Glyphs use the loaded font's visible ink bounds, with a small upward optical adjustment. Until the intro finishes, reserve letters scramble and counts show `?`; reduced motion uses static placeholders. The accessible description also waits until decoding ends before exposing the real reserve. Resumed games show their reserve immediately. Earlier designs remain under `artifacts/supply-mockups`; those images are historical mockups, not the source of game state. The queue's order remains hidden.
...
`WyrmDecoder.tsx` keeps the `waiting → enemy → tiles → ready` sequence. Its head follows a continuous orthogonal path; each square body piece follows the same path at its own distance behind the head, bending independently around right-angle corners. Body squares remain upright and the head changes direction in quarter turns. The head's arrival triggers each reveal once. Layout coordinates are cached until the viewport, element sizes or fonts change, so reveal bounces cannot move the route. The moving pieces share the life meter's shapes and dimensions; a straight final approach lays them into the measured resting positions without scaling or morphing. The static meter reserves its space while decoding, reduced motion reveals both stages quickly without travelling, and resumed games show the meter immediately.
