...
The [meaning-first review](docs/meaning-first-review.md) records the current rules, frozen dictionary and CHAOS publication. The [lexical and onboarding review](docs/lexical-onboarding-review.md) explains the earlier word-label pipeline, the short demo and the research behind it. The [gameplay review](docs/gameplay-onboarding-pass.md) preserves earlier player tests and records phone checks. The [ANGER review](docs/revive-candidate-notes.md) records the earlier bonus-based publication and its scoped rescue certificate; those proofs do not transfer to meaning-first puzzles.
...
Tests use Node's built-in TypeScript support; use Node 22.18+ or a newer supported release. New puzzles accept only spellings with a stored Open English Wordnet 2025 definition and a frozen puzzle meaning classification. Archived puzzles keep the bundled `an-array-of-english-words` validity rules. Gameplay makes no API requests and needs no LLM.
...
New meaning-first puzzles award **no word-type or length bonuses**. Every playable word has a frozen definition, selected sense, relation and reason, compiled from the complete starting/refill letter supply before solving. A word with no stored definition is rejected before it spends a life. Complete vocabulary coverage does not mean every possible relationship has been reviewed: neutral classifications mean no counter or reinforcing sense matched the reviewed enemy profile.
...
**September 25 now publishes CHAOS v10** (`letter-strike-7`): five lives, armoured A, Life R, Hit A, Revive A and 20 refill tiles. It has no grammar or length bonuses. Its frozen table stores definitions and classifications for all **29,647** accepted words that fit the total letter supply. The [selected review artifact](artifacts/meaning-v1/selected.json.gz) retains the exact encounter, solver evidence and opening certificate; the [publication review](docs/meaning-first-review.md) explains the selection and semantic checks. Every **3,041 opening spelling and all 7,999 physical tile selections** have a replayed familiar winning continuation, with no unsafe or unknown openings. This guarantees recovery after the first word; later choices can still lose.
...
Current **CHAOS v10** is rated **MEDIUM (estimate)**. Its review retains 30 winning strategies, including 10 that play after empty board slots appear, and scores 82.62 for generation quality. **CLEAR → SORT → HARMONY** is a three-word winning witness; **CHARM → SORT → ANGER → SYSTEM → HAND → CAT** is a six-word final-life win. Three words is an upper bound, not a proved minimum. The complete opening certificate is separate from shortest-route proof and does not claim that every later choice is recoverable.
...
**GENERATOR / SOLVER** opens the separate generated-puzzle review browser, also available directly at `/?generator=1`. Its initial review is the published CHAOS v10 encounter, with the same frozen table used by gameplay. New batches start with CHAOS, seed `meaning-review`, a 20-tile initial finite budget and Revive enabled. New candidates carry frozen definition-backed meanings and disable grammar/LONG. Review cards distinguish archived rules, show vocabulary coverage, and let reviewers inspect any stored word’s definition, selected sense and classification reason. The generator API keeps its original padded-supply default unless `refillLimit` is supplied. `npm run generate -- --count 200 --enemy MELANCHOLY` runs a local batch; omit `--enemy` for automatic suitability-based selection. See the [generator guide](docs/generator.md) for APIs, mutation, quality metrics, search limits and JSON export, and the [five-candidate review report](docs/melancholy-generated-review.md) for earlier recorded results. Generation never publishes candidates automatically; dated publications use explicit frozen catalog entries.

The offline pipeline stores Open English Wordnet 2025 senses, definitions, inflected forms and source provenance. Reviewed enemy profiles use pinned senses and bounded lexical expansion; exclusions prevent unrelated senses such as diligence from becoming calmness counters. `compilePuzzleMeanings` enumerates every definition-backed spelling that fits the total letter supply, independently of solver budgets, and freezes the resulting table into the encounter. Preview, submission, word discovery and terminal-state checks use the same table. Validation rejects missing or stale compilations; supply-changing mutations must recompile. The older POS pipeline remains available for archived rules. See the [generator guide](docs/generator.md) for rebuilding and the [lexical review](docs/lexical-onboarding-review.md) for the historical DIRTY/WORRY diagnosis.
