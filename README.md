...
**September 24, 2026 uses the highest-ranked generated MELANCHOLY candidate**, selected explicitly from the review set (score 69.53). Its frozen runtime snapshot is `src/daily/puzzles/2026-09-24.json`, with puzzle version 5 and the existing `letter-strike-4` rules. New and untouched attempts use it; already-started and completed attempts retain the original version. Other dates retain the authored catalog. Each date has its own attempt and result; published content is deterministic and independent of player state or later generator rankings.
