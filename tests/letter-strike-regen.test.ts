...
test('REGEN construction and mutation are opt-in, deterministic, and JSON replayable', () => {
  ...
  const candidate = createCandidate('MELANCHOLY', seed, { includeRegenTile: true, archivedSpecialTiles: true })
  assert.deepEqual(candidate, createCandidate('MELANCHOLY', seed, { includeRegenTile: true, archivedSpecialTiles: true }))
