...
test('every saved generated board reproduces from its seed, including selected refinements', () => {
  for (const { candidate } of saved) {
    if (!candidate.provenance.parentId) {
      assert.deepEqual(jsonValue(createCandidate(candidate.enemyWord, candidate.seed, { lexicalMode: 'legacy', archivedSpecialTiles: true })), candidate)
      continue
    }
    ...
    const original = createCandidate(candidate.enemyWord, rootSeed, { lexicalMode: 'legacy', archivedSpecialTiles: true })
