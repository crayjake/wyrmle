...
The current publication is **September 25 CHAOS v10** (`letter-strike-7`), with 20 refills, armoured A, Life R, Hit A and Revive A. Its 29,647 definition-backed words are frozen into the encounter. The [selected artifact](../artifacts/meaning-v1/selected.json.gz) contains the full review and certificate; the [meaning-first publication review](meaning-first-review.md) records the semantic audit, replayable routes and selection evidence.
...
Current **CHAOS v10** certifies **all 3,041 valid opening spellings and 7,999 physical tile selections** against its full frozen dictionary. Every opening has a familiar winning continuation; unsafe and unknown counts are both zero. This covers the first word, including alternative choices of matching normal or special tiles. It does not certify arbitrary later decisions or a minimum winning depth. The complete certificate is retained in the [selected artifact](../artifacts/meaning-v1/selected.json.gz), rather than duplicated in the DEV review bundle.
