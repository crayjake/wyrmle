...
import data from './data/semantic-refinements-v1.json' with { type: 'json' }
...
export type RefinementManifest = {
  ...
  threshold: 0.5; trustedDecisionBases: string[]
}
...
export function semanticRefinementInput(manifest: RefinementManifest, enemyWord: string, enemyDefinition: string, senses: readonly RefinementSense[]) {
...
function validateManifest(manifest: RefinementManifest): void {
  ...
  const keys = new Set<string>([...strings, 'threshold', 'trustedDecisionBases'])
  if (!manifest || Object.keys(manifest).some(key => !keys.has(key)) || strings.some(key => !nonempty(manifest[key]))
    || manifest.threshold !== 0.5 || !Array.isArray(manifest.trustedDecisionBases)
    || JSON.stringify([...manifest.trustedDecisionBases].sort()) !== JSON.stringify(trustedBases)) {
...
export function createSemanticRefinementProvider(input: RefinementCache, base: BaseProvider = semanticAssessmentProvider,
  vocabulary: readonly string[] = getDefinedDictionaryWords()) {
  ...
  return Object.freeze({
    refine(enemy: string, inventory: Readonly<Record<string, PuzzleWordMeaning>>): RefinementResult {
      ...
      for (const word of Object.keys(inventory).sort()) {
        ...
        if (basis && trustedBases.includes(basis)) continue
        if (!['local-vector-nli', 'local-vector-nli-neutral'].includes(basis ?? '')) {
          issues.push(`${word}: unsupported baseline decision basis`); continue
        }
        if (basis === 'local-vector-nli-neutral' && row[3].length === 0) continue
        eligible.push(word)
        const senses = row[3].map(sense => input.senses[sense])
        const inputDigest = semanticRefinementDigest(semanticRefinementInput(manifest, enemy, base.definition(enemy), senses))
        ...
        const sense = senses.find(sense => sense.id === memo.senseId)
        if (memo.relation === 'neutral' ? memo.senseId !== meaning.senseId : !sense) {
          issues.push(`${word}: LLM decision does not select an admitted qualified sense`); continue
        }
        words[word] = Object.freeze({ ...meaning,
          ...(memo.relation !== 'neutral' && sense ? { definition: sense.definition, lemma: sense.lemma, senseId: sense.id,
            source: sense.source, partsOfSpeech: Object.freeze([...new Set([...meaning.partsOfSpeech, sense.partOfSpeech])]) } : {}),
          relation: memo.relation === 'neutral' ? 'unrelated' : memo.relation, reason: memo.explanation,
          assessment: readWordSemanticAssessment({ ...meaning.assessment!, decisionBasis: 'local-llm' }),
        })
      ...
      const metadata = readSemanticRefinementMetadata({ version: manifest.version, modelId: manifest.modelId,
        ...
        cacheDigest: semanticRefinementDigest({ eligible, reviewed }), eligibleWords: eligible.length, reviewedWords: reviewed.length })
...
// An empty package keeps draft authoring available while every release gate fails closed.
const cache = data as unknown as Omit<RefinementCache, 'words'> & { words?: string[] }
