...
function CandidateReview({ ranked, onPlay }: { ranked: RankedCandidate; onPlay: Props['onPlay'] }) {
  ...
  return <article className="generator-review" aria-label={`Review ${candidate.id}`}>
    <header className="generator-review-heading">
      <div><p className="generator-kicker">{candidate.goal.archetypes.join(' · ')}</p><h3>{candidate.enemyWord}</h3>
    ...
    <div className="generator-layout">
      <section aria-label="Starting position">
        ...
        <p>{encounter.startingResolve} starting Resolve · {Object.entries(encounter.grammarModifiers ?? {}).map(([part, amount]) => `${part} ${amount! > 0 ? '+' : ''}${amount} strike`).join(', ') || 'No grammar weakness'}
          {encounter.longWordRule ? ` · LONG ${encounter.longWordRule.minimumLength}+ adds ${encounter.longWordRule.bonusStrikes}` : ''}</p>
      </section>
      <section aria-label="Analysis overview">
        ...
        <dl className="generator-metrics">
          ...
          <Metric label="Best Resolve remaining">{analysis.maximumResolveRemaining ?? 'Unknown'}</Metric>
    ...
    <section aria-label="Fairness and suspense">
      ...
      <dl className="generator-metrics generator-metrics-wide">
        ...
        <Metric label="Final-Resolve rescue rate, assessed">{percent(analysis.penultimateRescueRate)}</Metric>
    ...
    <section aria-label="Mechanic importance">
      ...
      <div className="generator-table-wrap"><table><thead><tr><th>Disabled mechanic</th><th>Importance</th><th>Changed replay outcomes</th><th>Best alternate win</th><th>Evidence</th></tr></thead>
        <tbody>{analysis.counterfactuals.map(item => <tr key={item.mechanic}><th>{item.mechanic}</th><td>{decimal(item.importance)}</td>
      ...
      {analysis.specialTileDecisions && <details><summary>Special-tile preservation and timing</summary>
        {(['ward', 'strike'] as const).map(gem => {
          ...
          return <p key={gem}><strong>{gem.toUpperCase()}</strong>: {decision.reasonableOpeningWordsUsing} reasonable opening words use it;
    ...
    <details open={!validation.accepted}>
      ...
      {validation.reasons.length > 0 && <ul className="generator-rejected">{validation.reasons.map(reason => <li key={reason.code}>{reason.message}</li>)}</ul>}
      {validation.warnings.length > 0 && <ul>{validation.warnings.map(warning => <li key={warning.code}>{warning.message}</li>)}</ul>}
    </details>
    ...
    <details><summary>Semantic anchors &amp; grammar annotations · spoilers</summary>
      <div className="generator-table-wrap"><table><thead><tr><th>Anchor</th><th>Role</th><th>Expected</th><th>Familiarity</th></tr></thead><tbody>
        {candidate.anchors.map(anchor => <tr key={anchor.word}><th>{anchor.word}</th><td>{anchor.roles.join(', ')}</td><td>{anchor.expected}</td><td>{percent(anchor.commonness)}</td></tr>)}
      </tbody></table></div>
      ...
      <details><summary>Explicit part-of-speech annotations</summary><pre>{JSON.stringify(encounter.wordPartsOfSpeech, null, 2)}</pre></details>
    </details>
    <details><summary>Winning lines ({analysis.winningLines.length}) · spoilers</summary>
      ...
      {analysis.winningLines.map((line, index) => <details key={index}><summary>Line {index + 1}: {line.turns} turns, {line.resolveRemaining} Resolve remaining</summary><MoveList moves={line.moves} /></details>)}
    ...
    <details><summary>Clutch / rescue lines ({analysis.clutchLines.length}) · spoilers</summary>
      {analysis.clutchLines.map((line, index) => <details key={index}><summary>Rescue {index + 1}: Resolve 1, remaining {line.remainingLetters} · {line.winningMoves.length} winning move(s)</summary>
      ...
      {!analysis.clutchLines.length && <p>No final-Resolve rescue witnessed within this search.</p>}
    </details>
    <details><summary>Enemy-letter opportunities</summary>
      <div className="generator-table-wrap"><table><thead><tr><th>Position</th><th>Hits</th><th>Board copies</th><th>Refill copies (upper bound)</th><th>Observed word routes</th><th>Categories</th></tr></thead><tbody>
        {analysis.letterOpportunityCounts.map(letter => <tr key={letter.position}><th>{letter.position + 1}: {letter.letter}{analysis.criticalSinglePointLetters.includes(letter.position) ? ' ⚠' : ''}</th>
          <td>{letter.requiredHits}</td><td>{letter.currentBoardCopies}</td><td>{letter.reachableRefillCopies}</td><td>{letter.observedMatchingMoves}</td><td>{letter.observedCategories.join(', ')}</td></tr>)}
      </tbody></table></div>
    ...
    <details><summary>Quality score breakdown &amp; analysis scope</summary>
      <div className="generator-table-wrap"><table><thead><tr><th>Component</th><th>Value</th><th>Weight</th><th>Points</th><th>Reason</th></tr></thead><tbody>
        {quality.components.map(component => <tr key={component.name}><th>{component.name}</th><td>{component.value.toFixed(2)}</td><td>{component.weight}</td><td>{component.contribution.toFixed(2)}</td><td>{component.explanation}</td></tr>)}
      </tbody></table></div><ul>{analysis.notes.map(note => <li key={note}>{note}</li>)}</ul>
...
export default function DevGenerator({ onPlay, onClose }: Props) {
  ...
  return <dialog className="generator-dialog" ref={dialog} aria-labelledby="generator-title" onCancel={event => { event.preventDefault(); onClose() }}>
    ...
    <p>Generate, inspect, and play candidates with the real letter-strike rules. Accepted results enter a human review queue; daily puzzles are published separately.</p>
    ...
    <div className="generator-browser">
      <aside className="generator-results" aria-label="Ranked candidates">
        ...
        <ol>{visible.map((item, index) => <li key={item.candidate.id}><button type="button" aria-pressed={item.candidate.id === selectedId} onClick={() => setSelectedId(item.candidate.id)}>
          <span><strong>#{index + 1} {item.candidate.enemyWord}</strong><b>{item.quality.total.toFixed(1)}</b></span><small>{item.candidate.seed}</small><small>{item.candidate.goal.archetypes.join(' · ')}</small>
